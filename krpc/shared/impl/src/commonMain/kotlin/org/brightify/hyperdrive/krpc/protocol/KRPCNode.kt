...
class RPCExtensionServiceRegistry(extensions: List<RPCExtension>): ServiceRegistry {
    ...
    suspend fun run() = withContext(coroutineContext) {
        when (val handshakeResult = handshakePerformer.performHandshake(connection)) {
            is RPCHandshakePerformer.HandshakeResult.Success -> {
                ...
                extendedServiceRegistry = MutableConcatServiceRegistry(
                    RPCExtensionServiceRegistry(extensions),
